...
public class ObjetAwtFenetre implements InterfaceGraphic {
	...
	public void creerFenetre(String titre, int largeur, int hauteur){
		...
		getFrame().setLocationRelativeTo(null);
		getFrame().setLayout(new GridLayout(1,1));
		getFrame().setVisible(true);
	}
	

	public void genererRepere(int xMax, int yMax){
		int cm = Utilitaire.getInstance().getCentimetre();
		this.dessinerSegment(xMax/2, 0, xMax/2, yMax, Color.LIGHT_GRAY);
		this.dessinerSegment(0, yMax/2, xMax, yMax/2, Color.LIGHT_GRAY);
		for(int i = cm; i < xMax; i += cm)
			this.dessinerSegment(i, yMax/2-5, i, yMax/2+5, Color.LIGHT_GRAY);
		for(int j = cm; j < yMax; j += cm)
			this.dessinerSegment(xMax/2-5, j, xMax/2+5, j, Color.LIGHT_GRAY);
	}
	...
	public void dessinerCercle(int x, int y, int rayon, Color color) {
		try{
			...
			getGraphique().fillOval(x, y, rayon, rayon);
